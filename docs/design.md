...
### ⚠️ 4.2.1 ปัญหาที่พบตอนสร้าง `hook.js` — การสแกนเลขพัสดุจะเด้งไปหน้า not_found

**เกิดอะไรขึ้น:** ช่องที่รับ STOP คือ `#txt_imei` ช่องเดียวกับที่รับ IMEI (design §5.2)
เมื่อพนักงานสแกนบาร์โค้ดบนกล่อง หน้าเดิมจะยิงค่านั้นเข้า `get_order_new` ตามปกติ
API หาไม่เจอ แล้ว **เด้งไปหน้า `/shopee/imei/not_found/:imei` ทุกออเดอร์**

นี่ไม่ใช่ปัญหาเล็ก — 1,500 ครั้งต่อวัน พนักงานถูกเด้งไปหน้าที่ไม่ต้องการทุกครั้งที่ปิดกล่อง

**⚠️ ต้องดักที่ `keyup` ไม่ใช่ `keydown`** — ตรวจไฟล์จริงแล้วพบว่า
`imei_new_api.ejs:158` ผูกไว้กับ `onkeyup="validate(this, event)"` ไม่ใช่ `onkeydown`
การเรียก `preventDefault()` ตอน keydown **ไม่หยุด keyup ที่ตามมา** การดักจึงไม่ทำงานเลย
ถ้าดักผิดจังหวะ · ตัวรับแบบ capture ที่ `document` ทำงานก่อน handler ที่ตัว element เสมอ
`stopImmediatePropagation()` จึงหยุด `onkeyup` ของหน้าเดิมได้จริง

**`#txt_close` ใช้รับการสแกนปิดคลิปไม่ได้** — `airways_hot_test2.ejs:305` ผูก
`onkeyup="close_bill()"` ซึ่งเด้งกลับทันทีเมื่อกดปุ่มอะไรก็ได้ ไม่ใช่เฉพาะ Enter
การสแกนที่ช่องนี้จึง redirect ตั้งแต่ตัวอักษรแรก · การสแกนปิดคลิปต้องทำที่ `#txt_imei`
หลังหน้าเด้งกลับ `/shopee/imei` ตามที่ §5.2 ออกแบบไว้

**สิ่งที่ทำไปแล้ว:** `hook.js` ดัก Enter ทั้ง keydown และ keyup ในชั้น capture แล้ว **หยุดไม่ให้หน้าเดิมยิง API**
เฉพาะเมื่อค่าที่สแกน "ไม่ใช่ IMEI แน่ๆ" (มีตัวอักษรและยาว ≥8 หลังตัดอักขระคั่น)
ค่าที่เป็นตัวเลขล้วนยังปล่อยให้ระบบเดิมตัดสินตามเดิมทุกกรณี

| | |
|---|---|
| ควบคุมด้วย | ค่าคงที่ `INTERCEPT_TRACKING` ใน `hook.js` — ปิดได้โดยไม่ต้อง deploy sellcenter |
| กระทบระบบเดิมไหม | เฉพาะค่าที่ระบบเดิมจะตอบ "ไม่พบ" อยู่แล้ว — ไม่ได้เปลี่ยนผลลัพธ์ของ IMEI ที่ถูกต้อง |
| ขัดกับ §7 ไหม | §7 ระบุว่าไม่เปลี่ยนพฤติกรรมของ `GetOrderByImeiNew` และการ redirect — ข้อนี้ไม่ได้แตะทั้งสองอย่าง แต่**ทำให้บางค่าไปไม่ถึง API** ซึ่งเกินกว่าที่ §7 เขียนไว้ตรงๆ |

**ต้องตัดสินใจที่ Gate 1** — ตัวเลือกที่มี:

| | ข้อดี | ข้อเสีย |
|---|---|---|
| **ก. ดักไว้ (ทำไปแล้ว)** | พนักงานไม่ถูกเด้ง ไม่ต้องแก้ sellcenter | hook มีผลต่อการทำงานของหน้าเดิม แม้เฉพาะค่าที่จะล้มเหลวอยู่ดี |
| ข. ปล่อยให้เด้ง | hook ไม่แตะพฤติกรรมเดิมเลย | +1 หน้าเสียเวลาต่อออเดอร์ × 1,500/วัน — น่าจะไม่ผ่านเกณฑ์ NFR-1.2 |
| ค. ย้ายช่องรับ STOP ไปหน้าต่างอัด | แยกขาดจากระบบเดิมสมบูรณ์ | พนักงานต้องคลิกเปลี่ยนโฟกัสก่อนสแกนทุกครั้ง |

ตอนนี้ใช้ **ก.** เป็นค่าเริ่มต้นเพราะเป็นทางเดียวที่ไม่เพิ่มภาระให้พนักงาน
วัดผลจริงที่ Gate 1 แล้วค่อยยืนยัน
