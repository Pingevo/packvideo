...
## ที่เก็บคลิปบนเครื่องพัฒนา

ตั้ง `PACK_VIDEO_PATH` ให้อยู่ **นอกโฟลเดอร์โปรเจกต์** เสมอ — เครื่องพัฒนาใช้

```
PACK_VIDEO_PATH=~/packvideo-data
```

เคยตั้งเป็น `./data/pack_video` แล้วคลิปที่อัดจริงถูกลบทิ้งตอนเก็บกวาดหลังทดสอบ
เพราะโฟลเดอร์ในโปรเจกต์ถูกปฏิบัติเหมือนพื้นที่ชั่วคราว · ตอนนี้ระบบเตือนตอนบูต
ถ้าตรวจพบว่าที่เก็บอยู่ในโฟลเดอร์โปรเจกต์

**ฐานข้อมูลไม่ได้หายไปด้วย** — แถวของคลิปยังอยู่ครบ แต่ไฟล์วิดีโอกู้ไม่ได้
ระบบจะแสดงว่า "ไม่มีไฟล์วิดีโอแล้ว" ซึ่งแยกจาก "ถูกลบตามนโยบาย 30 วัน" คนละกรณี
