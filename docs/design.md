...
| **D2** | อัดเป็น **MP4/H.264 ตรงจากเบราว์เซอร์** ถ้ารองรับ | ไม่ต้อง transcode ตอนเก็บ — แปลงเฉพาะตอน export | ✅ ตัดความเสี่ยง "transcode 1,500 คลิป/วัน" ออกจาก critical path |
| **D3** | **อัปโหลดเป็นชิ้น (chunk) ระหว่างอัด** ไม่ใช่ก้อนเดียวตอนจบ | FR-1.6 อัดยาวไม่จำกัด → ก้อนเดียวกินแรมและหายทั้งคลิปถ้าเบราว์เซอร์แครช | ✅ เรื่องที่เดิมไม่ได้คิด และจำเป็นเพราะ FR-1.6 |
...
> ผลของ D2: งานแปลงไฟล์ย้ายออกจากเส้นทางหลักทั้งหมด ความเสี่ยง "transcode 1,500 คลิป/วัน"
> ใน design doc §9 กลายเป็นเรื่องรองทันที **แต่ต้องยืนยันว่าเบราว์เซอร์ที่โต๊ะแพ็ครองรับจริง
> ตั้งแต่วันแรกของ pilot** ถ้าไม่รองรับ ต้องกลับมาวางแผนเครื่องแปลงไฟล์

### 7.2 อัปโหลดเป็นชิ้น (D3)

```mermaid
graph LR
  MR["MediaRecorder<br/>timeslice 5000ms"] -->|"ondataavailable"| Q["คิวในหน่วยความจำ"]
  Q -->|"PUT chunk/:seq"| API
...
| เรื่อง | การออกแบบ |
|---|---|
| ขนาดชิ้น | 5 วินาที ≈ 0.6 MB ที่ 1.0 Mbps |
| ต่อชิ้นได้เลย | MediaRecorder ผลิต fragmented stream ต่อท้ายกันได้ตรงๆ |
| อัปซ้ำ | `PUT` ตาม `seq` — ยิงซ้ำได้ ไม่เกิดข้อมูลซ้อน |
| เน็ตขาด | คิวลง IndexedDB ทนได้ ≥ 10 นาที (NFR-1.4) |
| แครชกลางคลิป | เสียเฉพาะ ≤5 วินาทีสุดท้าย ไม่ใช่ทั้งคลิป |
...
2. ยืนยัน D2 — ทดสอบ `MediaRecorder.isTypeSupported('video/mp4;codecs=avc1')` บนเบราว์เซอร์
   ที่โต๊ะแพ็คจริง **ก่อนเริ่มเฟส 2a** เพราะถ้าไม่ผ่านต้องวางแผนเครื่องแปลงไฟล์เพิ่ม
3. `/sc:workflow` → แผนลงมือรายเฟส
